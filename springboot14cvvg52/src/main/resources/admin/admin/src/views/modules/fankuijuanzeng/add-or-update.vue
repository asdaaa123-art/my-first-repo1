<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="感想" prop="ganxiang" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.ganxiang" label="感想" prop="ganxiang" >
				<span class="text">{{ruleForm.ganxiang}}</span>
			</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.pingjia" label="评价" prop="pingjia" >
				<span class="text">{{ruleForm.pingjia}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `fankuijuanzeng/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fankuijuanzengCrossAddOrUpdateFlag = false;
											this.parent.search();

<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"18px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('juanzengjilu','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('juanzengjilu','捐赠状态统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						捐赠状态统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('juanzengjilu','捐赠次数统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						捐赠次数统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","width":"100%","padding":"0","fontSize":"16px","color":"#000","background":"none"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="juanzengzhezhanghao"
						label="捐赠者账号">
						<template slot-scope="scope">
							{{scope.row.juanzengzhezhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="juanzengzhexingming"
						label="捐赠者姓名">
						<template slot-scope="scope">
							{{scope.row.juanzengzhexingming}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="juanzengzhaopian" width="200" label="捐赠照片">
						<template slot-scope="scope">
							<div v-if="scope.row.juanzengzhaopian">
								<img v-if="scope.row.juanzengzhaopian.substring(0,4)=='http'&&scope.row.juanzengzhaopian.split(',w').length>1" :src="scope.row.juanzengzhaopian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.juanzengzhaopian)">
								<img v-else-if="scope.row.juanzengzhaopian.substring(0,4)=='http'" :src="scope.row.juanzengzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.juanzengzhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.juanzengzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.juanzengzhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="shouzhuzhezhanghao"
						label="受助者账号">
						<template slot-scope="scope">
							{{scope.row.shouzhuzhezhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="shouzhuzhexingming"
						label="受助者姓名">
						<template slot-scope="scope">
							{{scope.row.shouzhuzhexingming}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="juanzengliebiao"
						label="捐赠列表">
						<template slot-scope="scope">
							{{scope.row.juanzengliebiao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="juanzengzhuangtai"
						label="捐赠状态">
						<template slot-scope="scope">
							{{scope.row.juanzengzhuangtai}}
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('juanzengjilu','接受捐赠')" @click="jieshoujuanzengCrossAddOrUpdateHandler(scope.row,'cross','是','','juanzengzhuangtai','已接收','已接收,审核中'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								接受捐赠
							</el-button>
							<el-button class="edit" v-if=" isAuth('juanzengjilu','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="btn8" v-if="isAuth('juanzengjilu','私聊')" type="success" @click="chatClick(scope.row)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								私聊
							</el-button>
		...
		<jieshoujuanzeng-cross-add-or-update v-if="jieshoujuanzengCrossAddOrUpdateFlag" :parent="this" ref="jieshoujuanzengCrossaddOrUpdate"></jieshoujuanzeng-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="juanzengzhuangtaiChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="juanzengzhexingmingChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import jieshoujuanzengCrossAddOrUpdate from "../jieshoujuanzeng/add-or-update";
	...
	export default {
		...
		data() {
			return {
				...
				jieshoujuanzengCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			jieshoujuanzengCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			jieshoujuanzengCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.jieshoujuanzengCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','juanzengjilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jieshoujuanzengCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jieshoujuanzengCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var juanzengzhuangtaiChart1 = echarts.init(document.getElementById("juanzengzhuangtaiChart1"),'macarons');
					this.$http({
						url: "juanzengjilu/group/juanzengzhuangtai",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].juanzengzhuangtai);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].juanzengzhuangtai
								})
							}
							var option = {};
							let titleObj = this.bar.title
							titleObj.text = '捐赠状态统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							juanzengzhuangtaiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								juanzengzhuangtaiChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var juanzengzhexingmingChart2 = echarts.init(document.getElementById("juanzengzhexingmingChart2"),'macarons');
					this.$http({
						url: "juanzengjilu/group/juanzengzhexingming",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].juanzengzhexingming);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].juanzengzhexingming
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '捐赠次数统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							juanzengzhexingmingChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								juanzengzhexingmingChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				if(this.searchForm.juanzengzhuangtai!='' && this.searchForm.juanzengzhuangtai!=undefined){
					params['juanzengzhuangtai'] = this.searchForm.juanzengzhuangtai
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "juanzengjilu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {

	...
	<div class="main-containers">
		<div class="body-containers">
			<div class="top-container">
				<!-- info -->
				<div class="top_title">
					<span @click="goMenu('/index/home')">爱心小屋捐赠系统的设计与实现</span>
				</div>
		...
		<el-dialog title="投诉建议" :visible.sync="chatFormVisible" width="600px" :before-close="chatClose">
	...
	methods: {
		...
		async getSession() {
			await this.$http.get(`${localStorage.getItem('UserTableName')}/session`, {emulateJSON: true}).then(async res => {
				if (res.data.code == 0) {
					if(localStorage.getItem('UserTableName')== 'juanzengzhe') {
						localStorage.setItem('username', res.data.data.juanzengzhezhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'shouzhuzhe') {
						localStorage.setItem('username', res.data.data.shouzhuzhezhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'zhiyuanzhe') {
						localStorage.setItem('username', res.data.data.zhiyuanzhezhanghao);
					}
					if(localStorage.getItem('UserTableName')== 'kefurenyuan') {
						localStorage.setItem('username', res.data.data.kefuzhanghao);
					}

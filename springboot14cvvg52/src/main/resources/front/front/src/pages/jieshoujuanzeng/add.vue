		...
		<el-form
			...
			<el-form-item class="add-item" label="捐赠时间" prop="juanzengshijian">
				<el-input v-model="ruleForm.juanzengshijian" 
					placeholder="捐赠时间" clearable :disabled=" false  ||ro.juanzengshijian"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="接收时间" prop="jieshoushijian">
				<el-date-picker
					:disabled=" false  ||ro.jieshoushijian"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`jieshoujuanzeng/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jieshoujuanzeng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="所需物资" prop="suoxuwuzi">
				<el-input v-model="ruleForm.suoxuwuzi" 
					placeholder="所需物资" clearable :disabled=" false  ||ro.suoxuwuzi"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="捐赠照片" v-if="type!='cross' || (type=='cross' && !ro.juanzengzhaopian)" prop="juanzengzhaopian">
				<file-upload
					tip="点击上传捐赠照片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="捐赠照片" prop="juanzengzhaopian">
				<img v-if="ruleForm.juanzengzhaopian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.juanzengzhaopian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.juanzengzhaopian.split(',')" :src="baseUrl+item">
			</el-form-item>
			...
			<el-form-item class="add-item" label="捐赠列表" prop="juanzengliebiao">
				<el-input v-model="ruleForm.juanzengliebiao" 
					placeholder="捐赠列表" clearable :disabled=" false  ||ro.juanzengliebiao"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="捐赠时间" prop="juanzengshijian">
				<el-date-picker
					:disabled=" false  ||ro.juanzengshijian"
			...
			<el-form-item class="add-item"  label="捐赠状态" prop="juanzengzhuangtai">
				<el-select v-model="ruleForm.juanzengzhuangtai" placeholder="请选择捐赠状态" :disabled=" false  ||ro.juanzengzhuangtai" >
					<el-option
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='juanzengzhaopian'){
							this.ruleForm.juanzengzhaopian = obj[o]?obj[o].split(",")[0]:'';
				...
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.juanzengzhezhanghao!=''&&json.juanzengzhezhanghao) || json.juanzengzhezhanghao==0){
							this.ruleForm.juanzengzhezhanghao = json.juanzengzhezhanghao;
							this.ro.juanzengzhezhanghao = true;
						}
						if((json.juanzengzhexingming!=''&&json.juanzengzhexingming) || json.juanzengzhexingming==0){
			...
			juanzengzhaopianUploadChange(fileUrls) {
				this.ruleForm.juanzengzhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>

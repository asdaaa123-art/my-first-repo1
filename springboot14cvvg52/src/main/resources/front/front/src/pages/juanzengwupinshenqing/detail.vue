	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/juanzengwupinshenqing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">情况说明</div>
						<div class="text "  >{{detail.qingkuangshuoming}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('juanzengwupinshenqing','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('juanzengwupinshenqing','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('juanzengwupinshenqing','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('juanzengwupinshenqing','捐赠')" 
							@click="onAcross('juanzengjilu','是','','','')" type="warning">
							捐赠
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('juanzengwupinshenqing','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.shouzhuzhexingming;
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`juanzengwupinshenqing`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/juanzengwupinshenqing', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/juanzengwupinshenqingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此捐赠物品申请？') .then(_ => {
					this.$http.post('juanzengwupinshenqing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			chatClick() {
				...
				if(localStorage.getItem('frontSessionTable')=='shouzhuzhe') {
					if(this.detail.shouzhuzhezhanghao == user.shouzhuzhezhanghao){
						this.$message.error('不能给自己发信息')
						return false
					}
				}
				this.$http.get('shouzhuzhe/query', {
					params: {
						shouzhuzhezhanghao: this.detail.shouzhuzhezhanghao
					}
				}).then(res => {
					if (res.data && res.data.code == 0) {
						this.fid = res.data.data.id
						this.initWebSocket(this.fid)
						this.fname = res.data.data.shouzhuzhezhanghao
						if (res.data.data.touxiang) {
							this.fpic = res.data.data.touxiang.split(',')[0]
						} else if (res.data.data.headportrait) {
							this.fpic = res.data.data.headportrait.split(',')[0]

	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/kefurenyuan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('kefurenyuan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('kefurenyuan','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('kefurenyuan','私聊')&&detail.id!=mid" @click="chatClick">私聊</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.kefuxingming;
						...
						if(res.data.data.kefuzhanghao){
							this.fname = res.data.data.kefuzhanghao
						}
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/kefurenyuan', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/kefurenyuanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此客服人员？') .then(_ => {
					this.$http.post('kefurenyuan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',

	...
	export default {
		data() {
			return {
				...
				rules: {
					shouzhuzhezhanghao: [
						{ required: true, message: '受助者账号不能为空', trigger: 'blur' },
					],
					mima: [
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`shouzhuzhe/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shouzhuzhe/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shouzhuzhe?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shouzhuzhe','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shouzhuzhe','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('shouzhuzhe','私聊')&&detail.id!=mid" @click="chatClick">私聊</el-button>
						<el-button class="auditBtn" v-if="btnAuth('shouzhuzhe','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(res.data.data.shouzhuzhezhanghao){
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/shouzhuzhe', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/shouzhuzheAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此受助者？') .then(_ => {
					this.$http.post('shouzhuzhe/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
